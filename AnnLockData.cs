...
Inputs:             
Parameters:	        None 
Outputs:	        Annual Lock data	
Description:	    data layer to check lock or update
Detailed Design:    None 
Other:	            Called by: frmTabAnnualBeaWorksheet
...
namespace CprsDAL
{
    public class AnnLockData
    {
        //check whether the tc has been locked or not
        public string GetAnnLock(string tc)
        {
            string locked_by = "";
            try
            {
                ...
                string Query = "SELECT USRNME from dbo.AnnLOCK WHERE TC2 =" + GeneralData.AddSqlQuotes(tc);
        ...
        //Update Annlock table, if locktable = true, update usrnme with user name
        ...
        public bool UpdateTabLock(string tc, bool locktable)
        {
            ...
            {
                sql_connection.Open();
                string usql = "UPDATE dbo.ANNLOCK SET " +
                                "USRNME = @USRNME " +
                                "WHERE TC2 = @TC2";
