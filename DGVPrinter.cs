    ...
    public class DGVPrinter
    {
        ...
        private float HeaderHeight 
        {
            get
            {
                ...
                // already included
                headerheight += TitleHeight+ SubTitleHeight + UserInfoHeight;
        ...
        // Infoline properties
        #region Info line properties

        // override flat
        bool overrideuserinfoformat = false;

        // formatted height of infoline
        float userinfoheight = 0;

        /// <summary>
        /// Infoline for this report. Default is empty.
        /// </summary>
        ...
        /// <summary>
        /// Font for the userinfo. Default is Tahoma, 12pt.
        /// </summary>
        private Font userinfofont;
        public Font UserInfoFont
        {
            get { return userinfofont; }
            set { userinfofont = value; }
        }

        /// <summary>
        /// Foreground color for the userinfo. Default is Black
        /// </summary>
        private Color userinfocolor;
        public Color UserinfoColor
        {
            get { return userinfocolor; }
            set { userinfocolor = value; }
        }

        /// <summary>
        /// Allow override of the header cell format object
        /// </summary>
        private StringFormat userinfoformat;
        public StringFormat UserInfoFormat
        {
            get { return userinfoformat; }
            set { userinfoformat = value; overrideuserinfoformat = true; }
        }

        /// <summary>
        /// Allow the user to override the userinfo string alignment. Default value is 
        /// Alignment - Near; 
        /// </summary>
        public StringAlignment UserInfoAlignment
        {
            get { return userinfoformat.Alignment; }
            set
            {
                userinfoformat.Alignment = value;
                overrideuserinfoformat = true;
            }
        }

        /// <summary>
        /// Allow the user to override the userinfo string format flags. Default values
        /// are: FormatFlags - NoWrap, LineLimit, NoClip
        /// </summary>
        public StringFormatFlags UserInfoFormatFlags
        {
            get { return userinfoformat.FormatFlags; }
            set
            {
                userinfoformat.FormatFlags = value;
                overrideuserinfoformat = true;
            }
        }

        /// <summary>
        /// Control where in the document the info prints
        /// </summary>
        private PrintLocation userinfoprint = PrintLocation.All;
        public PrintLocation UserInfoPrint
        {
            get { return userinfoprint; }
            set { userinfoprint = value; }
        }
        ...
        private float UserInfoHeight
        {
            get
            {
                if (PrintLocation.All == UserInfoPrint)
                    return userinfoheight + userinfospacing;

                if ((PrintLocation.FirstOnly == UserInfoPrint) && (1 == CurrentPage))
                    return userinfoheight + userinfospacing;

                if ((PrintLocation.LastOnly == UserInfoPrint) && (totalpages == CurrentPage))
                    return userinfoheight + userinfospacing;
        ...
        /// Mandatory spacing between the grid and the footer
        /// </summary>
        private float userinfospacing;
        public float UserInfoSpacing
        {
            get { return userinfospacing; }
            set { userinfospacing = value; }
        }

        /// <summary>
        /// Title Block Background Color
        /// </summary>
        private Brush userinfobackground;
        public Brush UserInfoBackground
        {
            get { return userinfobackground; }
            set { userinfobackground = value; }
        }

        /// <summary>
        /// Title Block Border
        /// </summary>
        private Pen userinfoborder;
        public Pen UserInfoBorder
        {
            get { return userinfoborder; }
            set { userinfoborder = value; }
        }
        ...
        public DGVPrinter()
        {
            ...
            buildstringformat(ref subtitleformat, null, StringAlignment.Near, StringAlignment.Near,
                StringFormatFlags.NoWrap | StringFormatFlags.LineLimit | StringFormatFlags.NoClip, StringTrimming.Word);
            buildstringformat(ref userinfoformat, null, StringAlignment.Near, StringAlignment.Near,
                StringFormatFlags.NoWrap | StringFormatFlags.LineLimit | StringFormatFlags.NoClip, StringTrimming.Word);
        ...
        public void PrintDataGridViewWithoutDialog(DataGridView dgv)
        {
            if (EnableLogging) Logger.LogInfoMsg("PrintDataGridView process started");
            if (null == dgv) throw new Exception("Null Parameter passed to DGVPrinter.");
            if (!(typeof(DataGridView).IsInstanceOfType(dgv)))
        ...
        private void measureprintarea(Graphics g)
        {
            ...
            if (PrintHeader)
            {
                ...
                userinfoheight = (g.MeasureString(userinfo, userinfofont, printWidth, userinfoformat)).Height;
        ...
        private int Pagination()
        {
            ...
            staticheight = pageHeight - FooterHeight - pagesets[currentpageset].margins.Bottom - 20;
            ...
            for (int currentrow = 0; currentrow < (rowstoprint.Count); currentrow++)
            {
                ...
                if (newpage != paging.keepgoing)
                {
                    ...
                    staticheight = pageHeight - FooterHeight - pagesets[currentpageset].margins.Bottom-20;//PrintMargins.Bottom;
        ...
        private bool PrintPage(Graphics g)
        {
            ...
            if (PrintHeader)
            {
                ...
                // print user info line if provided, & we're not skipping it
                if (0 != UserInfoHeight && !String.IsNullOrEmpty(userinfo))
                {
                    /*print user id */
                    printsection(g, ref printpos, userinfo, userinfofont,
                        userinfocolor, userinfoformat, overrideuserinfoformat,
                        pagesets[currentpageset],
                        userinfobackground, userinfoborder);

                    /*print date time */
                    String strDate = DateTime.Now.ToLongDateString() + " " + DateTime.Now.ToShortTimeString();
                    printsection(g, ref printpos, strDate, userinfofont,
                        userinfocolor, pagenumberformat, overrideuserinfoformat,
                        pagesets[currentpageset],
                        userinfobackground, userinfoborder);

                    // account for line spacing
                    printpos += UserInfoHeight*2;
                }
        ...
        private void printRsection(Graphics g, ref float pos, string text,
            Font font, Color color, StringFormat format, bool useroverride, PageDef pageset,
            Brush background, Pen border)
        {
            // measure string
            SizeF printsize = g.MeasureString(text, font, pageset.printWidth, format);
            float txtwidth = g.MeasureString(text, font, pageset.printWidth).Width;

            // build area to print within
            RectangleF printarea = new RectangleF((float)(pageset.margins.Left+(pageset.printWidth -txtwidth)), pos, (float)pageset.printWidth,
               printsize.Height);
